...
class EPUBBookLoader(BaseBookLoader):
    ...
    def _process_combined_paragraph(self, p_block, index, p_to_save_len, thread_safe=False):
        text = []
        ...
        if send_num > 1:
            ...
            for p in p_list:
                ...
                if self.single_translate and self.block_size > 0:
                    ...
                    index = self._process_paragraph(p, new_p, index, p_to_save_len, thread_safe=False)
                    print()
            ...
            if self.single_translate and self.block_size > 0 and len(p_block) > 0:
                index = self._process_combined_paragraph(p_block, index, p_to_save_len, thread_safe=False)
    ...
    def set_parallel_workers(self, workers):
        ...
        if workers > 8:
            print(f"⚠️  Warning: {workers} workers is quite high. Consider using 2-8 workers for optimal performance.")
    ...
    def _process_chapter_parallel(self, chapter_data):
        ...
        try:
            ...
            if send_num > 1:
                ...
                self._translate_paragraphs_acc_parallel(p_list, send_num, thread_translator,
                                                       chapter_context_list, chapter_translated_list)
            else:
                ...
                for p in p_list:
                    ...
                    if self.resume and index < p_to_save_len:
                        ...
                        t_text = self._translate_with_chapter_context(
                            thread_translator, new_p.text, chapter_context_list, chapter_translated_list
                        )
                        if type(p) == NavigableString:
                            new_p = t_text
                            with self._progress_lock:
    ...
    def _translate_with_chapter_context(self, translator, text, chapter_context_list, chapter_translated_list):
    ...
    def _translate_paragraphs_acc_parallel(self, p_list, send_num, translator, chapter_context_list, chapter_translated_list):
        ...
        class ChapterHelper:
            def __init__(self, parent_loader, translator, context_list, translated_list):
            ...
            def deal_old(self, wait_p_list, single_translate):
                ...
                try:
                    ...
                    self.translator.context_translated_list = self.translated_list.copy()
            ...
            def deal_new(self, p, wait_p_list, single_translate):
                ...
                self.parent_loader.helper.insert_trans(
                    p, translation, self.parent_loader.translation_style, single_translate
                )

        chapter_helper = ChapterHelper(self, translator, chapter_context_list, chapter_translated_list)
        ...
        try:
            ...
            if self.enable_parallel and len(document_items) > 1:
                ...
                if effective_workers < self.parallel_workers:
                    print(f"📊 Optimized workers: {effective_workers} (reduced from {self.parallel_workers})")
                else:
                    print(f"📊 Using {effective_workers} workers")

                if self.accumulated_num > 1:
                    print(f"📝 Each chapter applies accumulated_num={self.accumulated_num} independently")

                if self.context_flag:
                    print(f"🔗 Context enabled: each chapter maintains independent context (limit={self.translate_model.context_paragraph_limit})")
                else:
                ...
                chapter_pbar = tqdm(total=len(document_items), desc="Chapters", unit="ch")
                ...
                with ThreadPoolExecutor(max_workers=effective_workers) as executor:
                    future_to_item = {
                        executor.submit(self._process_chapter_parallel, chapter_data): chapter_data[0]
                    ...
                    for future in as_completed(future_to_item):
                        ...
                        try:
                            ...
                            chapter_pbar.set_postfix_str(f"Latest: {item.file_name[:20]}...")
